...
class WikidataService extends WikidataConceptSchemeSet {
  ...
  async getMapping(_id) {
    ...
    let mapping
    if (claim) {
      mapping = mappers.mapMappingClaims({ [claim.mainsnak.property]: [claim] }, { from: mappers.mapIdentifier(qid), schemes: this })[0]
    }
    if (!mapping) {
 ...
 */
WikidataService.prototype.suggest = require("./suggest")
